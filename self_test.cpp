...
struct nonprintable {};
static_assert(is_detected_v<print_result_type, decltype(any_of(0,0))>);
static_assert(!is_detected_v<print_result_type, decltype(any_of(0, nonprintable{}))>);
static_assert(is_detected_v<print_result_type, decltype(none_of(0,0))>);
static_assert(!is_detected_v<print_result_type, decltype(none_of(0, nonprintable{}))>);
static_assert(is_detected_v<print_result_type, decltype(all_of(0,0))>);
static_assert(!is_detected_v<print_result_type, decltype(all_of(0, nonprintable{}))>);
...
// TODO operator() must return an op_t of bound func/values
#if 0
...
//TODO handle rvalues in a way that makes sense, whatever that is
#if 0
...
static_assert(throwing_tuple || std::is_nothrow_constructible_v<decltype(all_of(1,1)), int, int>);
static_assert(throwing_tuple || noexcept(all_of(1,2)));
static_assert(!noexcept(all_of(1,throwing(3))));
//TODO correct noexcept signature for operator()
#if 0
static_assert(std::is_nothrow_invocable_v<std::equal_to<>, decltype(all_of(1,2)), int>);
...
int main()
{
  ...
  test tests[] = {
      ...
      {
          "all_of is false with false member",
          [] {
            int v = 1;
